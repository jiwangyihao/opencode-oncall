...
import {
  ...
  runCommand,
  runWechatSlashThroughRealOpencode,
  sendKeys,
...
const OPENCODE_WECHAT_INTERACTIVE_ARGS = ["--agent", "build"]
...
test("real host PTY helper: spawnRealOpencodePty appends commandArgsOverride after runtime dispatch args", async () => {
  ...
  const session = await spawnRealOpencodePty({
    ...
    commandArgsOverride: OPENCODE_WECHAT_INTERACTIVE_ARGS,
  ...
  try {
    ...
    assert.deepEqual(session.args, [
      ...
      "call",
      "C:/Tools/opencode.cmd",
      ...OPENCODE_WECHAT_INTERACTIVE_ARGS,
    ])
...
test("real host PTY helper: spawnRealOpencodePty uses windows binary runtime directly", async () => {
  ...
  try {
    ...
    assert.deepEqual(session.args, OPENCODE_WECHAT_INTERACTIVE_ARGS)
...
test("real host PTY helper: resolveRealHostPluginInlineConfigContent merges plugin dist entry with disabled MCP overrides", async () => {
  const inlineConfigContent = await resolveRealHostPluginInlineConfigContent({
    ...
    artifact: {
      entryFilePath: "C:/repo/opencode-wechat/dist/index.js",
    },
  ...
  assert.deepEqual(JSON.parse(inlineConfigContent), {
    plugin: ["C:/repo/opencode-wechat/dist/index.js"],
    mcp: {
      context7: { enabled: false },
    },
  })
...
test("real host PTY helper: WeChat slash helper sends /recover through current slash surface", async () => {
  const dataEmitter = new EventEmitter()
  const exitEmitter = new EventEmitter()
  const sentInputs = []
  const fakePty = {
  ...
  const result = await runWechatSlashThroughRealOpencode({
    host: {
      hostRoot: "C:/tmp/opencode-host",
      cacheRoot: "C:/tmp/opencode-host/cache",
      configRoot: "C:/tmp/opencode-host/config",
    ...
    inlineConfigContent: JSON.stringify({ plugin: ["C:/repo/opencode-wechat/dist/index.js"] }),
    spawnPtyImpl: () => fakePty,
    readScreenImpl: async () => buffers[Math.min(readCount++, buffers.length - 1)],
    inputChangeTimeoutMs: 20,
  })

  try {
    assert.equal(result.ok, true)
    assert.equal(result.stage, "wechat-slash-response-visible")
    assert.equal(result.slashCommand, "/recover")
    assert.match(result.askAnythingScreen, /Ask anything/i)
    assert.match(result.responseScreen, /没有可恢复的请求/)
    assert.deepEqual(sentInputs.slice(0, 2), ["/recover", "\r"])
  } finally {
